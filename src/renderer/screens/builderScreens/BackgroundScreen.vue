<template>
  <v-container fluid class="pa-4">
    ...
    <v-row>
      <!-- Left column: Background list table -->
      <v-col cols="7">
        <v-card variant="outlined" style="border-color: rgba(128, 128, 128, 0.2)">
          <v-card-title class="py-3">
            <v-row no-gutters @click="() => (isExpanded = !isExpanded)" class="align-center">
              <v-col class="" cols="4"> {{ t('BuildScreen.background') }} </v-col>
              <v-col class="text--secondary" cols="6">
                <v-text-field
                  ...
                  :dirty="
                    ...
                    characterStore.character.background.name.length > 0
                  "
                  @click:clear="onClear"
                ></v-text-field>
              </v-col>
        ...
        >
          <template v-if="selectedBackground">
            ...
            <div class="flex-grow-1 overflow-y-auto px-4 py-4 description-content">
              <div v-html="selectedBackground.htmlDescription"></div>
            </div>
          </template>
          <template v-else>
...
onMounted(() => {
  if (items.value && items.value.length > 0) {
    selectedBackground.value = items.value[0]
  }
})
</script>
