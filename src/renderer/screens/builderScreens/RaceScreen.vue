<template>
  <v-container fluid class="pa-4">
    ...
    <v-row>
                ...
                <v-text-field
                  ...
                  :dirty="
                    characterStore.character.race
                      ? characterStore.character.race.length > 0
                      : false
                  "
                  @click:clear="onClear"
                ></v-text-field>
      ...
      <!-- Right column: Race details pane -->
      <v-col cols="5">
        <v-card
          variant="outlined"
        ...
        >
          ...
          <template v-else>
            <div class="d-flex flex-column align-center justify-center fill-height text-grey py-12">
              <div class="text-subtitle-1">Select a race to view details</div>
            </div>
          </template>
        </v-card>
      </v-col>
...
onMounted(() => {
  if (items.value && items.value.length > 0) {
    selectedRace.value = items.value[0]
  }
})
</script>
